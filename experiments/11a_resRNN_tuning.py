#### CONDUCT GRID SEARCH TO TUNE HYPERPARAMETERS OF -- ResRNN (Model 4) -- ON SHASTA DATA (1944-2022) ####
...
def data_processing_resrnn(name, transform_type):
    """
...
def grid_search_resrnn(dataloader_train, dataloader_val):
    """Conduct grid search. Save results as csv"""

    # Define hyperparameter space
    names = ['hidden_size', 'dropout', 'random_seed']
    arrays = [[5, 10, 15, 20, 25, 30, 35, 40, 45, 50], 
    ...
    for i in range(grid.shape[0]):
        ...
        resRNN_tune = resRNN(input_size=input_size, hidden_size=hidden_size, output_size=output_size,
                             dropout_prob=dropout_prob)
        criterion = nn.MSELoss()
        optimizer = optim.Adam(resRNN_tune.parameters(), lr=0.001)

        # Run training loop and get validation error
        train_losses, val_losses = training_loop(model=resRNN_tune, criterion=criterion, optimizer=optimizer, 
    ...
    # Save results
    results.to_csv('report/results/hyperparameter_tuning/resRNN_tuning.csv')
    return


## TRAIN AND SAVE MODEL WITH OPTIMAL HYPERPARAMETERS ##

def train_optimal_model_resrnn(dataloader_train, dataloader_val):
    ...
    grid_df = pd.read_csv('report/results/hyperparameter_tuning/resRNN_tuning.csv', index_col=0)
    ...
    grid_df_mean.to_csv('report/results/hyperparameter_tuning/resRNN_avg_tuning.csv')

    # Instantiate optiamal model
    input_size = 2
    hidden_size = int(grid_df_mean.iloc[0].hidden_size)
    output_size = 1
    dropout_prob = grid_df_mean.iloc[0].dropout

    torch.manual_seed(0)
    resRNN_model = resRNN(input_size=input_size, hidden_size=hidden_size, output_size=output_size,
                             dropout_prob=dropout_prob)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(resRNN_model.parameters(), lr=0.001)

    # Run training loop
    train_losses, val_losses = training_loop(model=resRNN_model, criterion=criterion, optimizer=optimizer, 
    ...
    plt.savefig('jobs/resRNN_training.png')

    # Save model
    torch.save(resRNN_model.state_dict(), 'src/models/saved_models/resRNN.pt')
    return

# run script
if __name__ == '__main__':
    # data processing
    dataloader_train, dataloader_val = data_processing_resrnn(name='Shasta', transform_type='standardize')

    # grid search
    grid_search_resrnn(dataloader_train=dataloader_train, dataloader_val=dataloader_val)

    # save optimal model
    train_optimal_model_resrnn(dataloader_train=dataloader_train, dataloader_val=dataloader_val)
