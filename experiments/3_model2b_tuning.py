#### CONDUCT (FINE) GRID SEARCH TO TUNE HYPERPARAMETERS OF -- MODEL 2 (b)  -- ON SHASTA DATA (1944-2022) ####
#### DATA PROCESSING: NORMALIZED, FILL NAN WITH TRAINING MEAN
#### LOSS FUNCTION: RMSLE LOSS
#### INPUTS: INFLOW, DOY
#### ALSO TRAINS AND SAVES OPTIMAL MODEL ####

#### CENTER SEARCH AROUND PARAMETERS FROM MODEL 1 COUNTERPART
...
from src.models.custom_loss import RMSLELoss


## DATA PROCESSING

def data_processing_2b(name, transform_type):
    """
...
## CONDUCT GRID SEARCH
def grid_search_2b(dataloader_train, dataloader_val):
    # Define hyperparameter space
    names = ['num_layers', 'hidden1', 'hidden2', 'dropout', 'random_seed']
    arrays = [[1, 2], [20, 25, 30], [20, 25, 30], 
            [0.3], [0, 10, 100, 1000, 10000]]
    ...
    for i in range(grid.shape[0]):
        ...
        model2b_tune = LSTMModel2(input_size=input_size, hidden_size1=hidden_size1, hidden_size2=hidden_size2, 
                                output_size=output_size, num_layers=num_layers, dropout_prob=dropout_prob, initial_output=0)
        criterion = RMSLELoss()
        optimizer = optim.Adam(model2b_tune.parameters(), lr=0.001)

        # Run training loop and get validation error
        train_losses, val_losses = training_loop(model=model2b_tune, criterion=criterion, optimizer=optimizer, 
    ...
    # Save results
    results.to_csv('report/results/hyperparameter_tuning/model2b_tuning.csv')


## SAVE AND TRAIN MODEL WITH OPTIMAL HYPERPARAMETERS
def train_optimal_model_2b(dataloader_train, dataloader_val):
    # Find optimal hyperparameters
    # Load in results from grid search
    grid_df = pd.read_csv('report/results/hyperparameter_tuning/model2b_tuning.csv', index_col=0)
    ...
    grid_df_mean.to_csv('report/results/hyperparameter_tuning/model2b_avg_tuning.csv')
    ...
    torch.manual_seed(0)
    model2b = LSTMModel2(input_size=input_size, hidden_size1=hidden_size1, hidden_size2=hidden_size2, 
                                output_size=output_size, num_layers=num_layers, dropout_prob=dropout_prob, initial_output=0)
    criterion = RMSLELoss()
    optimizer = optim.Adam(model2b.parameters(), lr=0.001)

    # Run training loop
    train_losses, val_losses = training_loop(model=model2b, criterion=criterion, optimizer=optimizer, 
    ...
    plt.savefig('jobs/model2b_training.png')

    # Save model
    torch.save(model2b.state_dict(), 'src/models/saved_models/model2b.pt')

# run script
if __name__ == '__main__':
    # data processing
    dataloader_train, dataloader_val = data_processing_2b(name='Shasta', transform_type='normalize')

    # grid search
    grid_search_2b(dataloader_train=dataloader_train, dataloader_val=dataloader_val)

    # save optimal model
    train_optimal_model_2b(dataloader_train=dataloader_train, dataloader_val=dataloader_val)