#### CONDUCT (FINE) GRID SEARCH TO TUNE HYPERPARAMETERS OF -- MODEL 3 (b)  -- ON SHASTA DATA (1944-2022) ####
...
## DATA PROCESSING
def data_processing_3b(name, transform_type):
...
## CONDUCT GRID SEARCH
def grid_search_3b(dataloader_train, dataloader_val):
    # Define hyperparameter space
    names = ['num_layers', 'hidden1', 'hidden2', 'dropout', 'random_seed']
    arrays = [[1, 2], [25, 30, 35], [10, 15, 20], 
    ...
    for i in range(grid.shape[0]):
        ...
        model3b_tune = LSTMModel3(input_size=input_size, hidden_size1=hidden_size1, hidden_size2=hidden_size2, 
                                output_size=output_size, num_layers=num_layers, dropout_prob=dropout_prob, initial_output=0, initial_implied_storage=0)
        criterion = RMSLELoss()
        optimizer = optim.Adam(model3b_tune.parameters(), lr=0.001)

        # Run training loop and get validation error
        train_losses, val_losses = training_loop(model=model3b_tune, criterion=criterion, optimizer=optimizer, 
    ...
    # Save results
    results.to_csv('report/results/hyperparameter_tuning/model3b_tuning.csv')
    return

## TRAIN AND SAVE MODEL WITH OPTIMAL HYPERPARAMETERS

def train_optimal_model_3b(dataloader_train, dataloader_val):
    ## Train model with optimal hyperparameters and save
    # Find optimal hyperparameters
    # Load in results from grid search
    grid_df = pd.read_csv('report/results/hyperparameter_tuning/model3b_tuning.csv', index_col=0)
    ...
    grid_df_mean.to_csv('report/results/hyperparameter_tuning/model3b_avg_tuning.csv')
    ...
    torch.manual_seed(0)
    model3b = LSTMModel3(input_size=input_size, hidden_size1=hidden_size1, hidden_size2=hidden_size2, 
                                output_size=output_size, num_layers=num_layers, dropout_prob=dropout_prob, initial_output=0, initial_implied_storage=0)
    criterion = RMSLELoss()
    optimizer = optim.Adam(model3b.parameters(), lr=0.001)

    # Run training loop
    train_losses, val_losses = training_loop(model=model3b, criterion=criterion, optimizer=optimizer, 
    ...
    plt.savefig('jobs/model3b_training.png')

    # Save model
    torch.save(model3b.state_dict(), 'src/models/saved_models/model3b.pt')

# run script
if __name__ == '__main__':
    # data processing
    dataloader_train, dataloader_val = data_processing_3b(name='Shasta', transform_type='normalize')

    # grid search
    grid_search_3b(dataloader_train=dataloader_train, dataloader_val=dataloader_val)

    # save optimal model
    train_optimal_model_3b(dataloader_train=dataloader_train, dataloader_val=dataloader_val)