#### CONDUCT (FINE) GRID SEARCH TO TUNE HYPERPARAMETERS OF -- MODEL 3 (a)  -- ON SHASTA DATA (1944-2022) ####
...
## DATA PROCESSING
def data_processing_3a(name, transform_type):
...
## CONDUCT GRID SEARCH
def grid_search_3a(dataloader_train, dataloader_val):
    ...
    for i in range(grid.shape[0]):
        ...
        model3a_tune = LSTMModel3(input_size=input_size, hidden_size1=hidden_size1, hidden_size2=hidden_size2, 
                                output_size=output_size, num_layers=num_layers, dropout_prob=dropout_prob, initial_output=0, initial_implied_storage=0)
        criterion = nn.MSELoss()
        optimizer = optim.Adam(model3a_tune.parameters(), lr=0.001)

        # Run training loop and get validation error
        train_losses, val_losses = training_loop(model=model3a_tune, criterion=criterion, optimizer=optimizer, 
    ...
    # Save results
    results.to_csv('report/results/hyperparameter_tuning/model3a_tuning.csv')
    return

## TRAIN AND SAVE MODEL WITH OPTIMAL HYPERPARAMETERS

def train_optimal_model_3a(dataloader_train, dataloader_val):
    ## Train model with optimal hyperparameters and save
    # Find optimal hyperparameters
    # Load in results from grid search
    grid_df = pd.read_csv('report/results/hyperparameter_tuning/model3a_tuning.csv', index_col=0)
    ...
    grid_df_mean.to_csv('report/results/hyperparameter_tuning/model3a_avg_tuning.csv')
    ...
    torch.manual_seed(0)
    model3a = LSTMModel3(input_size=input_size, hidden_size1=hidden_size1, hidden_size2=hidden_size2, 
                                output_size=output_size, num_layers=num_layers, dropout_prob=dropout_prob, initial_output=0, initial_implied_storage=0)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model3a.parameters(), lr=0.001)

    # Run training loop
    train_losses, val_losses = training_loop(model=model3a, criterion=criterion, optimizer=optimizer, 
    ...
    plt.savefig('jobs/model3a_training.png')

    # Save model
    torch.save(model3a.state_dict(), 'src/models/saved_models/model3a.pt')

# run script
if __name__ == '__main__':
    # data processing
    dataloader_train, dataloader_val = data_processing_3a(name='Shasta', transform_type='standardize')

    # grid search
    grid_search_3a(dataloader_train=dataloader_train, dataloader_val=dataloader_val)

    # save optimal model
    train_optimal_model_3a(dataloader_train=dataloader_train, dataloader_val=dataloader_val)