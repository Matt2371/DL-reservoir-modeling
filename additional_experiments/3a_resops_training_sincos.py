...
### ADD SIN-COS TRANSFORM OF DOY, SEE IF PERFORMANCE IMPROVES ###
...
def data_processing(res_id, transform_type, left, right='2020-12-31', train_frac=0.6, val_frac=0.2, test_frac=0.2, log_names=[], return_scaler=False, storage=False):
    ...
    doy = df.index.to_series().dt.dayofyear
    ...
    # Train/val/test tensors of shape (#chunks, chunksize, [inflow, outflow, storage, doy_sin, doy_cos])
    ts_train, ts_val, ts_test = pipeline.process_data(df) 

    # Separate inputs(X) and targets (y)
    if storage:
        X_train, X_val, X_test = ts_train[:, :, [0, 2, 3, 4]], ts_val[:, :, [0, 2, 3, 4]], ts_test[:, :, [0, 2, 3, 4]]
    else:
        X_train, X_val, X_test = ts_train[:, :, [0, 3, 4]], ts_val[:, :, [0, 3, 4]], ts_test[:, :, [0, 3, 4]]
...
def train_one_reservoir(res_id, left_year, storage):
    ...
    if storage:
        input_size = 4 # inflow, storage, doy_sin, doy_cos
    else:
        input_size = 3 # inflow, doy_sin, doy_cos
    hidden_size1 = 30
    hidden_size2 = 15
    ...
    # Save model
    os.makedirs(MODEL_SAVE_DIR, exist_ok=True)
    torch.save(model.state_dict(), os.path.join(MODEL_SAVE_DIR, f'resops_model{"1S" if storage else "1"}_{res_id}_sincos.pt'))
...
def main():
    ...
    df_r2.to_csv(os.path.join(SAVE_DIR, 'resops_individual_r2_sincos.csv'))
    ...
    df_wstorage_r2.to_csv(os.path.join(SAVE_DIR, 'resops_individual_storage_r2_sincos.csv'))
    return
